...
import type { FrontendLocaleData, LevelDefinition } from "./types";
import {
  afterNextRender,
  blankBeforePercent,
  formatNumber,
...
const getAngle = (value: number, min: number, max: number): number => {
  const percentage = getValueInPercentage(normalize(value, min, max), min, max);
  return (percentage * 180) / 100;
};

@customElement("ha-gauge-element")
export class HaGaugeElement extends LitElement {
  @property({ type: Number }) public min = 0;
  @property({ type: Number }) public max = 100;
  @property({ type: Number }) public value = 0;
  @property({ attribute: false }) public formatOptions?: Intl.NumberFormatOptions;
  ...
  private _getSegmentLabel(): string {
...
declare global {
  interface HTMLElementTagNameMap {
    "ha-gauge-element": HaGaugeElement;
  }
}
